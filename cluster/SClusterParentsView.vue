...
<template>
  <span v-if="cluster" :class="{ small: small }">
    <!-- -------------------- Parent -------------------- -->
    <!-- 1. Parent > Exist -->
    <span v-if="cluster.parent" class="ms-2">
      <!-- Parent of parent -->

      <span>
        <v-icon small>{{ cluster.parent_id ? "more_horiz" : "home" }}</v-icon>
      ...
      <!-- Parent has image -->
      <v-avatar v-if="cluster.parent.icon" :size="small ? 16 : 24"
        ><img :src="getShopImagePath(cluster.parent.icon, 64)" class="me-1"
      /></v-avatar>
      <!-- Parent has no image  -->
      <v-icon v-else small class="me-1">folder</v-icon>
    ...
    <!-- 1. Parent > Not exist -->
    <span v-else>
      <v-icon small class="me-1">home</v-icon>
