...
export default {
  ...
  props: {
    ...
    minHeight: { type: String, default: "42px" },
  },
      ...
      if (this.restrictedHourRange) {
        ...
        if (this.baseOn12Hours) {
          list = this.restrictedHourRange.map((hr) => {
            const l = hr.substring(0, hr.length - 1);
            const r = hr.substring(-1); // Todo: replace with slice! Check it!  substr -> substring
  ...
  methods: {
    ...
    onTab(column, value, evt) {
      if (this.appendToBody && evt.shiftKey) {
        ...
        if (
          firstValidValue &&
          firstValidValue.getAttribute("data-key") === String(value)
        ) {
          evt.preventDefault();
          // Focus back on <input>
    ...
    pasteHandler(evt) {
      ...
      if (this.inputIsEmpty) {
        ...
        this.kbInputLog = pastingText.substring(-2, 2); // Todo: replace with slice! Check it!  substr -> substring
        this.setKbInput();
    ...
    keyboardInput(newChar, isApm = false) {
      ...
      this.kbInputLog = `${this.kbInputLog.substring(-1)}${newChar}`; // Todo: replace with slice! Check it!  substr -> substring
      this.setKbInput();
      this.debounceClearKbLog();
    },
